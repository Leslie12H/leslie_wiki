...
- [报警日报证据丢失链路](domains/vidmuse/admin/pitfalls/monitoring-daily-brief-evidence-loss.md) — 输入与引用完整性、合法 supported 状态与读取枚举不一致、生产同窗读取验证、定时重试和已授权补发回读（2026-09-18）
...
- [Sand Eval CPU 与恢复取证入口](domains/sandai-data-smith/refs/sandeval-runtime-profile-evidence.md) — CFS、事件循环、SET/acquire、状态筛选、整包分页、后台化前后对比、流式分配中断及运行栈的证据边界（2026-09-25）。
