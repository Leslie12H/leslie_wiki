...
- [日报滚动部署启动交接](domains/vidmuse/admin/pitfalls/monitoring-daily-brief-startup-handoff.md) — 一次抢锁后跳过导致循环缺失，PR #878 与逐层验收指针（2026-09-14）
- [Tool 日汇总状态与已有数据展示](domains/vidmuse/admin/pitfalls/tool-daily-updating-hides-existing-data.md) — HTTP 成功后的数据门禁、dirty 与执行状态、部分日误挡及独立 Worker 核验（2026-09-14）
...
- [VidMuse 无库 Adapter 实施](domains/maxwell/projects/vidmuse-stateless-adapter-implementation.md) — AION 改动必要性复核：已有 Plugin ID/版本缓存、公共 SQL 与生命周期影响、capture/current 启用范围、未完成异步预留；五仓交付与 DEV 核验指针（2026-09-14）
