...
块数上限并不等于生成内容大小上限：高重叠率能把很短的源文档变成大量重复文本。检查 `chunkGenerationBudget` 是否在最终片段分配前累计 UTF-8 字节，并贯穿所有策略。导入的容量校验必须早于 `resolveBase`、原文件与 Asset 写入；仅清理文件不能消除超限导入留下的空知识库。
...
- HTTP 回归：`internal/app/api/http/knowledge_upload_test.go`；字节边界：`knowledge/application/content_limits_test.go`；分块上限及重切失败不覆盖旧数据：`knowledge/application/chunk_limits_test.go`；失败导入无空库、原文件或 Asset 副作用：`knowledge/application/import_preflight_test.go`。
