...
- 入口：`quality/application/resolution/manual_return_service.py` 的 restart；`quality/application/management/reinspection_service.py` 的 _rejected_members；`quality/application/inspection/review_query_service.py` 的 add_previous_items；前端 ReviewWorkspace.tsx 的 previousQueueTag。
