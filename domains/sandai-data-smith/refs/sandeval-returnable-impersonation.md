...
验收时查看 `platform/backend/tests/test_impersonation.py` 与 Redis CAS 回归，重点区分同一目标账号的独立登录和内部人员发起的会话。广播刷新不能替代服务端旧页面写入检查；认证准入审计也不能证明业务事务提交成功。

本次实现位于本地分支 `codex/returnable-impersonation`、提交 `f7d250f90`（2026-09-19）。隔离内存账号的浏览器/HTTP 验收只证明本地交互与控制边界；PR Gate、真实 Feishu OAuth、生产部署状态须另行核实。
