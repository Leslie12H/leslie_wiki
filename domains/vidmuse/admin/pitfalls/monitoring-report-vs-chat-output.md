...
## 2026-09-17 Current-Run draft preflight integration
...
## 2026-09-17 First-claim protocol binding and missing source notification action

**Why:** Counting every pending row as an active legacy Run blocks cutover on untouched historical events. Production claim_next_incident_event assigns the live protocol when reportTransport is absent, attempt_count is zero and response_json is empty. Separately, an SLS policy named Admin-derived can lose the actual Admin action while retaining Feishu/SMS/voice, so source cards appear without any Admin Incident.

**How to apply:** Match the cutover gate to the real first-claim predicate, include running/accepted/reconciling states and validate the configured report binding. Do not mutate old rows merely to clear the gate. For missing notifications, correlate exact Feishu message times, Admin lifecycle rows and the rule-bound action policy. Inspect both primary_policy_script and the graphical nodes. Restore only the missing action with an exact backup, concurrent-change check and inverse-diff verification; preserve existing notifications. An HTTP200 HTML WAF page is not webhook acceptance. Verify existing credentials against the internal handler without a valid alert payload, and separately require real source-delivery evidence before claiming end-to-end ingestion.

Evidence pointers: [guarded protocol cutover](https://github.com/world-sim-dev/vidmuse-admin/actions/runs/35215868710), [exact source policy](https://github.com/world-sim-dev/vidmuse-admin/actions/runs/35216128650), [script plus graph restoration](https://github.com/world-sim-dev/vidmuse-admin/actions/runs/35217550291). The policy modification timestamp establishes timing, not the actor or editing mechanism. Current preset/UI verification and missing historical replay boundaries remain in output/monitoring-prod-verification-20260916.md.
